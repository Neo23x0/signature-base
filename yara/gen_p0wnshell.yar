...
rule p0wnedPowerCat {
   meta:
      ...
      uuid = "059a8e58-7b7e-582e-ba4a-80e4dffe9b5e"
   strings:
...
rule Hacktool_Strings_p0wnedShell : FILE {
   meta:
      ...
      nodeepdive = 1
      uuid = "0846039d-1e00-5224-9560-55ab18034d54"
   strings:
      ...
      $fp5 = "use strict"
      uuid = "c9791804-4f08-5b7e-8d9d-37e2dfccec47"
   condition:
      filesize < 20MB
      and 1 of ($x*)
      and not 1 of ($fp*)
...
rule p0wnedPotato {
   meta:
      ...
      uuid = "2c2378e3-b948-5325-9afd-76424a7130b1"
   strings:
...
rule p0wnedExploits {
   meta:
      ...
      uuid = "9f754f5f-85e8-5b6f-bde2-566da4d39586"
   strings:
...
rule p0wnedListenerConsole {
   meta:
      ...
      uuid = "77d13c34-3e15-5bc1-a100-f04be38cfb44"
   strings:
...
rule p0wnedBinaries {
   meta:
      ...
      uuid = "0c62dd3a-195c-5890-b262-2eb00c58f8c1"
   strings:
...
rule p0wnedAmsiBypass {
   meta:
      ...
      uuid = "168af265-d3e9-59a2-b754-20d6c9a298b1"
   strings:
...
rule p0wnedShell_outputs {
   meta:
      ...
      super_rule = 1
      hash1 = "e1f35310192416cd79e60dba0521fc6eb107f3e65741c344832c46e9b4085e60"
      uuid = "c19fc14b-0c42-5dd1-bff2-ba75f4168d9c"
   strings:
