...
rule iexplore_ANOMALY
{
	...
	condition:
		filename == "iexplore.exe" and uint16 ( 0 ) == 0x5a4d and not filepath contains "teamviewer" and not 1 of ( $win* ) and not ( 
    ...
    (uint32be(0) == 0x44434401 and
     uint32be(12)== 0x50413330)    // magic :  DCD PA30 
		...
		and filepath contains "C : \\" and not filepath contains "Package_for_RollupFix"
}
...
rule svchost_ANOMALY
{
	...
	condition:
		filename == "svchost.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $win* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule explorer_ANOMALY
{
	...
	condition:
		filename == "explorer.exe" and uint16 ( 0 ) == 0x5a4d and not filepath contains "teamviewer" and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule sethc_ANOMALY
{
	...
	condition:
		filename == "sethc.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule Utilman_ANOMALY
{
	...
	condition:
		( filename == "utilman.exe" or filename == "Utilman.exe" ) and uint16 ( 0 ) == 0x5a4d and not 1 of ( $win* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule osk_ANOMALY
{
	...
	condition:
		filename == "osk.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule magnify_ANOMALY
{
	...
	condition:
		filename == "magnify.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $win* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule narrator_ANOMALY
{
	...
	condition:
		filename == "narrator.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $win* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule notepad_ANOMALY
{
	...
	condition:
		filename == "notepad.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $win* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule csrss_ANOMALY
{
	...
	condition:
		filename == "csrss.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule conhost_ANOMALY
{
	...
	condition:
		filename == "conhost.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule wininit_ANOMALY
{
	...
	condition:
		filename == "wininit.exe" and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule winlogon_ANOMALY
{
	...
	condition:
		filename == "winlogon.exe" and not 1 of ( $s* ) and uint16 ( 0 ) == 0x5a4d and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule SndVol_ANOMALY
{
	...
	condition:
		filename == "sndvol.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule doskey_ANOMALY
{
	...
	condition:
		filename == "doskey.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule lsass_ANOMALY
{
	...
	condition:
		filename == "lsass.exe" and uint16 ( 0 ) == 0x5a4d and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
...
rule taskmgr_ANOMALY
{
	...
	condition:
		( filename == "taskmgr.exe" or filename == "Taskmgr.exe" ) and not 1 of ( $s* ) and not ( 
    (uint32be(0) == 0x44434d01 and // magic :  DCM PA30
     uint32be(4) == 0x50413330)
    or
    (uint32be(0) == 0x44434401 and
     uint32be(12)== 0x50413330)    // magic :  DCD PA30 
		...
		and uint16 ( 0 ) == 0x5a4d and filepath contains "C : \\" and not filepath contains "Package_for_RollupFix"
}
...
rule APT_Cloaked_PsExec
{
	...
	condition:
		uint16(0)==0x5a4d and $s0 and $s1 and not filename matches /(psexec.exe|PSEXESVC.EXE|PsExec64.exe)$/is and not filepath matches /RECYCLE.BIN\\S-1/
}
...
rule SAM_Hive_Backup
{
	...
	condition:
		uint32(0)==0x66676572 and $s1 in (0..100) and not filename contains "sam.log" and not filename contains "SAM.LOG" and not filename contains "_sam" and not filename=="SAM" and not filename=="sam"
}
...
rule SUSP_Renamed_Dot1Xtray
{
	...
	condition:
		uint16(0)==0x5a4d and filesize <300KB and all of them and not filename matches /dot1xtray.exe/i and not filepath matches /Recycle.Bin/i
}
...
rule APT_Cloaked_CERTUTIL
{
	...
	condition:
		uint16(0)==0x5a4d and all of them and not filename contains "certutil" and not filename contains "CertUtil" and not filename contains "Certutil"
}
...
rule APT_SUSP_Solarwinds_Orion_Config_Anomaly_Dec20
{
	...
	condition:
		filename=="SolarWindows.Orion.Core.BusinessLayer.dll.config" and $s1 and not $fp1
}
