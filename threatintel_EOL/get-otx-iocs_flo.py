...
OTX_KEY = 'b09634843a2cf7f00a92a109256ed91ba68958740b981fbf3b1a078dd31b68cf'
...
# Only used as workaround as long as the getall() function retrieves pulses from authors that you're not subscribed to
AUTHORS = ['alienvault', 'the_good_guy', 'blueliv', 'cyberprotect', 'zer0daydan', 'nightingale']
...
class OTXReceiver():
    ...
    def write_iocs(self, ioc_folder):
        ...
        for event in self.events:
            try:
                # Check if whitelisted author
                if not any (author.lower() == event["author_name"].lower() for author in AUTHORS):
                    print("Author '%s' not in subscribed list" % event["author_name"])
                    continue
